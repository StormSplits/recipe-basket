...
export const InputQuestion = (props) => {
    ...
    const {promptData, setPromptData,setRecipes, setLoading,  fetchRecipies } = useContext(promptDataContext);
  ...
  return (
    ...
    <label>
      ...
      {index<9 && <button className="next" onClick={onClickHandler} disabled={promptData[name[index]].length <1}>Next &nbsp; <i className="fa-solid fa-check checkCenter"></i></button>}
      {index === 9 && <button className="next" type="submit" disabled={index===9 &&promptData[name[9]].length <1} >Show Recipies</button>}
      </div>
    </form>
  )
}
